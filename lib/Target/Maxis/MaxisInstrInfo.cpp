...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::AnalyzeCondBr:  Opc: " << Opc << ", Inst: Opc: " << Inst->getOpcode() << ' '; Inst->dump();
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::analyzeBranch " << __LINE__ << '\n'); 
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::BuildCondBr: MBB: "; MBB.dump(); dbgs() << " TBB: "; TBB->dump();); 
  unsigned Opc = Cond[0].getImm();
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::BuildCondBr: Cond[0]: Opc: " << Opc << ' '; Cond[0].dump(););
  const MCInstrDesc &MCID = get(Opc);
  MachineInstrBuilder MIB = BuildMI(&MBB, DL, MCID);
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::BuildCondBr: MIB.getInstr(): " << MIB.getInstr()->getOpcode() << ' ';
  MIB.getInstr()->dump(););
  ...
  MIB.addMBB(TBB);
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::BuildCondBr done: MBB: "; MBB.dump(); dbgs() << " TBB: "; TBB->dump();); 
}
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::insertBranch: ");
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::insertBranch: done: "; MBB.dump(); if (TBB) TBB->dump());
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::reverseBranchCondition: Cond[0]: " << Cond[0].getImm() <<
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::analyzeBranch " << __LINE__ << ": calling AnalyzeCondBr for last inst: ";
  LastInst->dump(););
    // Conditional branch
...
DEBUG_WITH_TYPE("axe", dbgs() << "MaxisInstrInfo::analyzeBranch " << __LINE__ << ": calling AnalyzeCondBr for second last inst: ";
  SecondLastInst->dump(););
